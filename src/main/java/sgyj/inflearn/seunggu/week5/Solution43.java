...
public class Solution43 {
    ...
    static int solution ( BufferedReader reader ) throws IOException {
        ...
        Deque<Patient> patients = new ArrayDeque<>();
        lines = reader.readLine().split( " ");
        for(int i=0; i<lines.length; i++) {
            patients.add(new Patient(i, Integer.parseInt(lines[i])));
        }
        int result = 0;
        int temp = -1;
        while(temp != m) {
            ...
            for(int i=0; i< patients.size(); i++) {
                Patient compare = patients.pop();
                if(flag) {
                    flag = patient.risk() >= compare.risk();
                }
                patients.add(compare);
            }
            if(flag) {
                temp = patient.order();
                result++;
    ...
    record Patient(int order, int risk) {
