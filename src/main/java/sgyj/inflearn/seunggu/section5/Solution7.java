...
public class Solution7 {
    ...
    static String solution ( BufferedReader reader ) throws IOException {
        ...
        String temp = lines[0];
        Deque<String> deque = new ArrayDeque<>( Arrays.asList( lines ).subList( 1, lines.length ) );
        lines = reader.readLine().split( "" );
        int cnt = 0;
        int fullSize = deque.size()+1;
        for(String line : lines ) {
            if(line.equals( temp )) {
                cnt++;
                if(!deque.isEmpty()) temp = deque.pop();
