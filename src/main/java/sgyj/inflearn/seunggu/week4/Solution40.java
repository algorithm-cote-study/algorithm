...
public class Solution40 {
     ...
     * @output : 한 줄에 쇠막대기와 레이저의 배치를 나타내는 괄호 표현이 공백없이 주어진다. 괄호 문자의 개수는 최대 100,000이다.
     */
    ...
    static int solution ( BufferedReader reader ) throws IOException {
        String[] sticks = reader.readLine().split( "");
        Deque<String> deque = new ArrayDeque<>();
        int result = 0;
        for(int i=0; i<sticks.length; i++) {
            if(sticks[i].equals( "(" )) deque.push(sticks[i]);
            else {
                deque.pop();
                if(sticks[i-1].equals( ")" )) result += 1;
